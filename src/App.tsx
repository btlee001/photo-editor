...
// --- COMPONENTS DEFINED OUTSIDE APP TO PREVENT RE-RENDERING ISSUES ---
...
const TouchSlider = ({ value, min, max, onChange, className }: { value: number, min: number, max: number, onChange: (val: number) => void, className?: string }) => {
  ...
  return (
    ...
    >
      ...
      <div 
        className="absolute w-8 h-8 bg-white border-2 border-blue-500 rounded-full shadow-md top-1/2 -translate-y-1/2 pointer-events-none"
...
// --- Extracted Handle Components for Crop ---
type HandleType = 'tl' | 'tr' | 'bl' | 'br' | 't' | 'b' | 'l' | 'r';

const Handle = ({ type, cursor, style, onDown }: { type: HandleType, cursor: string, style: React.CSSProperties, onDown: (t: HandleType, e: React.PointerEvent) => void }) => (
  <div 
    ...
    onPointerDown={(e) => onDown(type, e)}
  />
);

const SideHandle = ({ type, cursor, style, onDown }: { type: HandleType, cursor: string, style: React.CSSProperties, onDown: (t: HandleType, e: React.PointerEvent) => void }) => (
  <div 
  ...
  return (
    ...
    >
      ...
      >
        ...
        <SideHandle type="t" cursor="ns-resize" style={{ top: -20, left: 10, right: 10, height: 40 }} onDown={handlePointerDown} />
        <SideHandle type="b" cursor="ns-resize" style={{ bottom: -20, left: 10, right: 10, height: 40 }} onDown={handlePointerDown} />
        <SideHandle type="l" cursor="ew-resize" style={{ left: -20, top: 10, bottom: 10, width: 40 }} onDown={handlePointerDown} />
        <SideHandle type="r" cursor="ew-resize" style={{ right: -20, top: 10, bottom: 10, width: 40 }} onDown={handlePointerDown} />

        <Handle type="tl" cursor="nw-resize" style={{ top: -12, left: -12 }} onDown={handlePointerDown} />
        <Handle type="tr" cursor="ne-resize" style={{ top: -12, right: -12 }} onDown={handlePointerDown} />
        <Handle type="bl" cursor="sw-resize" style={{ bottom: -12, left: -12 }} onDown={handlePointerDown} />
        <Handle type="br" cursor="se-resize" style={{ bottom: -12, right: -12 }} onDown={handlePointerDown} />
      </div>

      <div className="absolute bottom-8 left-0 w-full flex justify-center gap-16 pointer-events-auto z-50">
        <button onClick={onClose} className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-red-500 shadow-2xl active:scale-95 transition-transform">
          <X size={32} />
        </button>
        <button onClick={onApply} className="w-16 h-16 bg-blue-500 rounded-full flex items-center justify-center text-white shadow-2xl active:scale-95 transition-transform">
          <Check size={32} />
        </button>
      </div>
    </div>
...
const Header = ({ imageSrc, onMenuClick, onAddClick, canvasRef }: any) => (
  <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 fixed top-0 w-full z-50 shadow-sm select-none">
    <div className="relative">
      <button onClick={onMenuClick} className="p-2 active:bg-gray-100 rounded-full transition-colors">
        <Menu className="text-gray-700 w-6 h-6" />
      </button>
    </div>
    {imageSrc && (
      <button onClick={onAddClick} className="p-2 active:bg-gray-100 rounded-full text-gray-600">
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <button onClick={onClose} className="text-gray-400 p-2 -mr-2 active:bg-gray-100 rounded-full">
          <X size={20} />
        </button>
      </div>
...
const Footer = ({ activeTool, onSelectTool }: any) => (
...
const AppModal = ({ menuContent, onClose }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-6 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-3xl w-full max-w-sm p-8 shadow-2xl scale-100 animate-in zoom-in-95">
        ...
        <button onClick={onClose} className="mt-8 w-full py-3.5 bg-gray-900 text-white rounded-2xl font-bold text-lg active:bg-gray-800 transition-colors shadow-md">
          关闭
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const applyMosaicEffect = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, type: 'pixel' | 'blur') => {
    ...
    if (type === 'pixel') ctx.fillRect(startX, startY, effectSize, effectSize);
    else { ctx.beginPath(); ctx.arc(x, y, effectSize / 2, 0, Math.PI * 2); ctx.fill(); }
  };
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col bg-gray-100 font-sans select-none overflow-hidden touch-none fixed inset-0">
      ...
      {showMenu && (
        <div className="absolute top-14 left-4 z-[70] w-48 bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden animate-in fade-in zoom-in duration-200">
      ...
      <AppModal menuContent={menuContent} onClose={() => setMenuContent('none')} />
          ...
          >
            ...
            {activeTool === 'crop' && (
              <CropOverlay 
                ...
                onClose={() => { setActiveTool('none'); setIsSettingsOpen(false); }}
                onApply={applyCrop}
              />
            )}
          </div>
      ...
      <SubToolBar 
        ...
        setMosaicType={setMosaicType}
      />

      <Footer activeTool={activeTool} onSelectTool={handleToolSelect} />
    </div>
  );
}